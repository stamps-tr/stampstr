...
#ifdef __cplusplus
  #ifdef pSimarray_char_10VTable_defined
    extern struct SimTypeVTable *pSimarray_char_10VTable;
  #else 
    struct SimTypeVTable *pSimarray_char_10VTable = NULL;
  #endif
#else
  struct SimTypeVTable *pSimarray_char_10VTable;
#endif

int array_char_10_to_string(const void *pValue, PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    if (pSimarray_char_10VTable != NULL
        && pSimarray_char_10VTable->m_pfnGetConvInfo(SptString, SptNone) == 1) {
       return pfnStrAppend(*(char**)pSimarray_char_10VTable->m_pfnToType(SptString, pValue), pStrObj);
    }
...
int check_array_char_10_string(const char *str, char **endptr)
{
    static array_char_10 rTemp;
    return string_to_array_char_10(str, &rTemp, endptr);
}

int string_to_array_char_10(const char *str, void *pValue, char **endptr)
{
    int nRet = 0;
    skip_whitespace(str);
    if (pSimarray_char_10VTable != NULL) {
        nRet = string_to_VTable(str, pSimarray_char_10VTable, pValue, endptr);
    }
    if (nRet == 0) {
        if (*str == '"')
...
int is_array_char_10_double_conversion_allowed()
{
    if (pSimarray_char_10VTable != NULL) {
        return is_VTable_double_conversion_allowed(pSimarray_char_10VTable);
    }
    return 0;
}

int array_char_10_to_double(const void *pValue, double *nValue)
{
    if (pSimarray_char_10VTable != NULL) {
        return VTable_to_double(pValue, pSimarray_char_10VTable, nValue);
    }
    return 0;
}

int is_array_char_10_long_conversion_allowed()
{
    if (pSimarray_char_10VTable != NULL) {
        return is_VTable_long_conversion_allowed(pSimarray_char_10VTable);
    }
    return 0;
}

int array_char_10_to_long(const void *pValue, long *nValue)
{
    if (pSimarray_char_10VTable != NULL) {
        return VTable_to_long(pValue, pSimarray_char_10VTable, nValue);
    }
    return 0;
}

void compare_array_char_10(int *pResult, const void *pValue1, const void *pValue2, SimTolerance *pTol, const char *pszPath, PFN_STR_LIST_APPEND pfnStrListAppend, void *pListErrPaths)
{
    int unitResult = 0;
    ...
    if (pSimarray_char_10VTable != NULL
        ...
        if (pSimarray_char_10VTable->m_version >= Scv65) {
            /* R15 and higher: VTable Compare function shall UPDATE *pResult global flag (*pResult |= SIM_CMP_RES_LT/...): */
            unitResult = pSimarray_char_10VTable->m_pfnCompare(pResult, pValue1, pValue2);
        } else {
            /* Before R15: VTable Compare function shall SET *pResult global flag (*pResult = -1/1/0): */
            pSimarray_char_10VTable->m_pfnCompare(&unitResult, pValue1, pValue2);
...
int get_array_char_10_signature(PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
...
SimTypeUtils _Type_array_char_10_Utils = {
    array_char_10_to_string,
    check_array_char_10_string,
    string_to_array_char_10,
    is_array_char_10_double_conversion_allowed,
    array_char_10_to_double,
    is_array_char_10_long_conversion_allowed,
    array_char_10_to_long,
