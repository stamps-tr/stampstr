...
int T_String_to_string(const void *pValue, PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    ...
    if (pConverter->m_bDisplayStrings)
        return pConverter->m_pfnCharArrayToString(pValue, kcg_char_to_string, 4, sizeof(kcg_char), pfnStrAppend, pStrObj, pConverter->m_bStringsPadding, pConverter->m_cStringsPaddingChar);
    else
        return pConverter->m_pfnArrayToString(pValue, kcg_char_to_string, 4, sizeof(kcg_char), pfnStrAppend, pStrObj);
}
...
int string_to_T_String(const char *str, void *pValue, char **endptr)
{
    ...
    if (nRet == 0) {
        if (*str == '"')
            nRet = pConverter->m_pfnStringToCharArray(str, pValue, &_Type_kcg_char_Utils, 4, sizeof(kcg_char), endptr, pConverter->m_bStringsPadding, pConverter->m_cStringsPaddingChar);
        else
            nRet = pConverter->m_pfnStringToArray(str, pValue, &_Type_kcg_char_Utils, 4, sizeof(kcg_char), endptr);
    }
    return nRet;
}
...
void compare_T_String(int *pResult, const void *pValue1, const void *pValue2, SimTolerance *pTol, const char *pszPath, PFN_STR_LIST_APPEND pfnStrListAppend, void *pListErrPaths)
{
    ...
    if (pSimT_StringVTable != NULL
        ...
        pConverter->m_pfnArrayComparison(pResult, pValue1, pValue2, 
                compare_kcg_char, 4, sizeof(kcg_char), pTol, pszPath, pfnStrListAppend, pListErrPaths);
    }
    if (unitResult != 0 && pfnStrListAppend != NULL && pszPath != NULL && *pszPath != 0 && pListErrPaths != NULL)
        pfnStrListAppend(pszPath, pListErrPaths);
}
...
{
    return pConverter->m_pfnGetArraySignature(get_kcg_char_signature, 4, pfnStrAppend, pStrObj); 
}
...
#ifdef __cplusplus
  #ifdef pSimT_uCharVTable_defined
    extern struct SimTypeVTable *pSimT_uCharVTable;
  #else 
    struct SimTypeVTable *pSimT_uCharVTable = NULL;
  #endif
#else
  struct SimTypeVTable *pSimT_uCharVTable;
#endif

int T_uChar_to_string(const void *pValue, PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    if (pSimT_uCharVTable != NULL
        && pSimT_uCharVTable->m_pfnGetConvInfo(SptString, SptNone) == 1) {
       return pfnStrAppend(*(char**)pSimT_uCharVTable->m_pfnToType(SptString, pValue), pStrObj);
    }
    return kcg_uint16_to_string(pValue, pfnStrAppend, pStrObj);
}

int check_T_uChar_string(const char *str, char **endptr)
{
    static T_uChar rTemp;
    return string_to_T_uChar(str, &rTemp, endptr);
}

int string_to_T_uChar(const char *str, void *pValue, char **endptr)
{
    int nRet = 0;
    skip_whitespace(str);
    if (pSimT_uCharVTable != NULL) {
        nRet = string_to_VTable(str, pSimT_uCharVTable, pValue, endptr);
    }
    if (nRet == 0) {
        nRet = string_to_kcg_uint16(str, pValue, endptr);
    }
    return nRet;
}

int is_T_uChar_double_conversion_allowed()
{
    if (pSimT_uCharVTable != NULL) {
        return is_VTable_double_conversion_allowed(pSimT_uCharVTable);
    }
    return is_kcg_uint16_double_conversion_allowed();
}

int T_uChar_to_double(const void *pValue, double *nValue)
{
    if (pSimT_uCharVTable != NULL) {
        return VTable_to_double(pValue, pSimT_uCharVTable, nValue);
    }
    return kcg_uint16_to_double(pValue, nValue);
}

int is_T_uChar_long_conversion_allowed()
{
    if (pSimT_uCharVTable != NULL) {
        return is_VTable_long_conversion_allowed(pSimT_uCharVTable);
    }
    return is_kcg_uint16_long_conversion_allowed();
}

int T_uChar_to_long(const void *pValue, long *nValue)
{
    if (pSimT_uCharVTable != NULL) {
        return VTable_to_long(pValue, pSimT_uCharVTable, nValue);
    }
    return kcg_uint16_to_long(pValue, nValue);
}

void compare_T_uChar(int *pResult, const void *pValue1, const void *pValue2, SimTolerance *pTol, const char *pszPath, PFN_STR_LIST_APPEND pfnStrListAppend, void *pListErrPaths)
{
    int unitResult = 0;
    /* Customized comparison */
    if (pSimT_uCharVTable != NULL
        && pSimT_uCharVTable->m_version >= Scv612
        && pSimT_uCharVTable->m_pfnCompare != NULL) {
        if (pSimT_uCharVTable->m_version >= Scv65) {
            /* R15 and higher: VTable Compare function shall UPDATE *pResult global flag (*pResult |= SIM_CMP_RES_LT/...): */
            unitResult = pSimT_uCharVTable->m_pfnCompare(pResult, pValue1, pValue2);
        } else {
            /* Before R15: VTable Compare function shall SET *pResult global flag (*pResult = -1/1/0): */
            pSimT_uCharVTable->m_pfnCompare(&unitResult, pValue1, pValue2);
            updateCompareResult(unitResult, pResult);
        }
    } else {
        /* Predefined comparison */
        compare_kcg_uint16(pResult, pValue1, pValue2, pTol, pszPath, pfnStrListAppend, pListErrPaths);
    }
    if (unitResult != 0 && pfnStrListAppend != NULL && pszPath != NULL && *pszPath != 0 && pListErrPaths != NULL)
        pfnStrListAppend(pszPath, pListErrPaths);
}

int get_T_uChar_signature(PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    return get_kcg_uint16_signature(pfnStrAppend, pStrObj);
}

int set_T_uChar_default_value(void *pValue)
{
    return set_kcg_uint16_default_value(pValue);
}

int init_T_uChar(void *pValue)
{
    return 1;
}

int release_T_uChar(void *pValue)
{
    return 1;
}

int copy_T_uChar(void *pToValue, const void *pFromValue)
{
    *((T_uChar*)pToValue) = *((T_uChar*)pFromValue);
    return 1;
}

SimTypeUtils _Type_T_uChar_Utils = {
    T_uChar_to_string,
    check_T_uChar_string,
    string_to_T_uChar,
    is_T_uChar_double_conversion_allowed,
    T_uChar_to_double,
    is_T_uChar_long_conversion_allowed,
    T_uChar_to_long,
    compare_T_uChar,
    get_T_uChar_signature,
    set_T_uChar_default_value,
    init_T_uChar,
    release_T_uChar,
    copy_T_uChar,
    sizeof(T_uChar)
...
#ifdef __cplusplus
  #ifdef pSimT_uShortVTable_defined
    extern struct SimTypeVTable *pSimT_uShortVTable;
  #else 
    struct SimTypeVTable *pSimT_uShortVTable = NULL;
  #endif
#else
  struct SimTypeVTable *pSimT_uShortVTable;
#endif

int T_uShort_to_string(const void *pValue, PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    if (pSimT_uShortVTable != NULL
        && pSimT_uShortVTable->m_pfnGetConvInfo(SptString, SptNone) == 1) {
       return pfnStrAppend(*(char**)pSimT_uShortVTable->m_pfnToType(SptString, pValue), pStrObj);
    }
    return kcg_uint8_to_string(pValue, pfnStrAppend, pStrObj);
}

int check_T_uShort_string(const char *str, char **endptr)
{
    static T_uShort rTemp;
    return string_to_T_uShort(str, &rTemp, endptr);
}

int string_to_T_uShort(const char *str, void *pValue, char **endptr)
{
    int nRet = 0;
    skip_whitespace(str);
    if (pSimT_uShortVTable != NULL) {
        nRet = string_to_VTable(str, pSimT_uShortVTable, pValue, endptr);
    }
    if (nRet == 0) {
        nRet = string_to_kcg_uint8(str, pValue, endptr);
    }
    return nRet;
}

int is_T_uShort_double_conversion_allowed()
{
    if (pSimT_uShortVTable != NULL) {
        return is_VTable_double_conversion_allowed(pSimT_uShortVTable);
    }
    return is_kcg_uint8_double_conversion_allowed();
}

int T_uShort_to_double(const void *pValue, double *nValue)
{
    if (pSimT_uShortVTable != NULL) {
        return VTable_to_double(pValue, pSimT_uShortVTable, nValue);
    }
    return kcg_uint8_to_double(pValue, nValue);
}

int is_T_uShort_long_conversion_allowed()
{
    if (pSimT_uShortVTable != NULL) {
        return is_VTable_long_conversion_allowed(pSimT_uShortVTable);
    }
    return is_kcg_uint8_long_conversion_allowed();
}

int T_uShort_to_long(const void *pValue, long *nValue)
{
    if (pSimT_uShortVTable != NULL) {
        return VTable_to_long(pValue, pSimT_uShortVTable, nValue);
    }
    return kcg_uint8_to_long(pValue, nValue);
}

void compare_T_uShort(int *pResult, const void *pValue1, const void *pValue2, SimTolerance *pTol, const char *pszPath, PFN_STR_LIST_APPEND pfnStrListAppend, void *pListErrPaths)
{
    int unitResult = 0;
    /* Customized comparison */
    if (pSimT_uShortVTable != NULL
        && pSimT_uShortVTable->m_version >= Scv612
        && pSimT_uShortVTable->m_pfnCompare != NULL) {
        if (pSimT_uShortVTable->m_version >= Scv65) {
            /* R15 and higher: VTable Compare function shall UPDATE *pResult global flag (*pResult |= SIM_CMP_RES_LT/...): */
            unitResult = pSimT_uShortVTable->m_pfnCompare(pResult, pValue1, pValue2);
        } else {
            /* Before R15: VTable Compare function shall SET *pResult global flag (*pResult = -1/1/0): */
            pSimT_uShortVTable->m_pfnCompare(&unitResult, pValue1, pValue2);
            updateCompareResult(unitResult, pResult);
        }
    } else {
        /* Predefined comparison */
        compare_kcg_uint8(pResult, pValue1, pValue2, pTol, pszPath, pfnStrListAppend, pListErrPaths);
    }
    if (unitResult != 0 && pfnStrListAppend != NULL && pszPath != NULL && *pszPath != 0 && pListErrPaths != NULL)
        pfnStrListAppend(pszPath, pListErrPaths);
}

int get_T_uShort_signature(PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    return get_kcg_uint8_signature(pfnStrAppend, pStrObj);
}

int set_T_uShort_default_value(void *pValue)
{
    return set_kcg_uint8_default_value(pValue);
}

int init_T_uShort(void *pValue)
{
    return 1;
}

int release_T_uShort(void *pValue)
{
    return 1;
}

int copy_T_uShort(void *pToValue, const void *pFromValue)
{
    *((T_uShort*)pToValue) = *((T_uShort*)pFromValue);
    return 1;
}

SimTypeUtils _Type_T_uShort_Utils = {
    T_uShort_to_string,
    check_T_uShort_string,
    string_to_T_uShort,
    is_T_uShort_double_conversion_allowed,
    T_uShort_to_double,
    is_T_uShort_long_conversion_allowed,
    T_uShort_to_long,
    compare_T_uShort,
    get_T_uShort_signature,
    set_T_uShort_default_value,
    init_T_uShort,
    release_T_uShort,
    copy_T_uShort,
    sizeof(T_uShort)
