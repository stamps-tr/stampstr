...
void AfterSimInit(void){
	...
	#endif
	pszBinListFiles[0] = "C:/Users/victo/Desktop/STAMPS/UserApplication/S2A/Simulation/S2A_UA_1_binaries.txt";
	pszBinListFiles[1] = 0;
	if (0/*OK*/ != A661ConnectServer("127.0.0.1", 1231, pszBinListFiles, "C:/Users/victo/Desktop/STAMPS/UserApplication/S2A/Simulation/S2A_UA_1_conf.xml" )) {
		const char *pszError;
...
void BeforeSimStep(void){
	buffer_el msg[S2A_UA_1_MAX_SIZE_INPUT_BUFFER];
	/* reveive from server */
	S2A_UA_1_receive_clear(&inputs_ctx, NULL);
	if (A661Receive(msg, sizeof msg) == 0)
		S2A_UA_1_receive(msg, sizeof msg, &inputs_ctx, NULL);
}


void AfterSimStep(void){
	static buffer_el msg[S2A_UA_1_MAX_SIZE_OUTPUT_BUFFER];
	int len;
	len = S2A_UA_1_send(msg, &outputs_ctx, NULL);
	/* Send to server */
	A661Send(msg, len);
}
